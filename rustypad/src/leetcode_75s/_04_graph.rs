...
impl Solution {
    ...
    fn courses_find_order(num_courses: i32, prerequisites: Vec<Vec<i32>>) -> Vec<i32> {
        // Use backtracking to find cycle present in the current path        
        fn cycle_present(
            graph: &HashMap<i32, Vec<i32>>, 
            node: i32, path: &mut Vec<i32>, 
            visited: &mut HashSet<i32>, ans: &mut Vec<i32>
        ) -> bool {
            if visited.contains(&node) {
                return true;
            }
            // Check for cycle in the current path
            if path[node as usize] == 1 {
                return true;
            }
            
            path[node as usize] = 1;
            // Visit the child node first and add into visited set
            if let Some(conn) = graph.get(&node) {
                for &children in conn {
                    if cycle_present(graph, node, path, visited, ans) {
                        return true;
                    }
                }
            }
            // Make the current node as visited and add it to ans
            visited.insert(node);
            ans.push(node);

            // Backtrack
            path[node as usize] = 0;
            false
        }
        ...
        for i in 0..num_courses {
            // This will check if any cycle for the current dfs traversal
            let mut path: Vec<i32> = vec![0; num_courses as usize];
            if !visited.contains(&i) && cycle_present(&graph, i, &mut path, &mut visited, &mut ans) {
                break;
            }
        }
        ans
