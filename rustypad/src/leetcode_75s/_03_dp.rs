...
impl Solution {
    ...
    pub fn can_jump(nums: Vec<i32>) -> bool {
        fn can_jump_helper(nums: &Vec<i32>, i: usize, cache: &mut HashMap<usize, bool>) -> bool {
            ...
            let end_pos = std::cmp::min(nums.len() - 1, nums[i] as usize + i);
            for j in i+1..end_pos+1 {
                if can_jump_helper(nums, j, cache) {
                    cache.insert(j, true);
                    return true;
                }
            }
        ...
        can_jump_helper(&nums, 0, &mut cache)
    }
