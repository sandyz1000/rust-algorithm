...
impl Solution {
    ...
    pub fn inorder_successor(root: TreeLink, p: TreeLink) -> TreeLink {
        let node_cell: &NodeRef = p.as_ref().unwrap();
        let p = node_cell.borrow().val;

        let mut res: TreeLink = None;
...
mod tests {
    ...
    #[test]
    fn test0() {
        let root: TreeLink = tree!(2, tree!(1), tree!(3));
        let p: TreeLink = tree!(1);
        let res: TreeLink = tree!(2);
        assert_eq!(Solution::inorder_successor(root, p), res);
        let root: TreeLink = tree!(5, tree!(3, tree!(2, tree!(1), None), tree!(4)), tree!(6));
        let p: TreeLink = tree!(6);
        let res: TreeLink = None;
        assert_eq!(Solution::inorder_successor(root, p), res);
    }
    ...
    fn test1() {
        ...
        let left: TreeLink = TreeLink::leaf(1);
        let right: TreeLink = TreeLink::leaf(3);
        let root: TreeLink = TreeLink::new(2, left, right);
    
        let p: TreeLink = TreeLink::leaf(1);
        let res: TreeLink = TreeLink::leaf(2);
        assert_eq!(Solution::inorder_successor(root, p), res);
    ...
    #[test]
    fn test_refcell() {
        let cv: &NodeRef = &Rc::new(RefCell::new(
