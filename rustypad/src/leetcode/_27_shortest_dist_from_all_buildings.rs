...
Check if every building can be reach from other building; if not then return -1
distance matrix to store the minimum distance from all building
bfs to add every node to visited
